...
Quosures enable *Programming on the Language*. This tutorial looks at way to
do that.

*Programming on the Language* is a way of circumventing how R evaluates an
expression and so that you that evaluation any way you want. This is called
non-standard evaluation or NSE, because you will not be evaluating the
the way that R does.

For example when R sees function call like...

`f(tbl, weight < 94, length > 12)`
...
to 'f'. And likewise for the third argument. This is called standard evaluation
because that is how R handles arguments in a function call.

The `dplyr::filter` function uses evaluates the same syntax in a completly
...
`dplyr::filter` doesn't evaluate each argument individually as R standard
evaluation would. Instead it interprets `weight` and `length` as columns of tbl
and then uses the second and third arguments as predicates used to choose rows
from `tbl`.

You can, of course, does same thing with R standard evaluation to do the same
thing, like this:
...
In the R syntax in this example the
...
create a syntax for arguments of a functions that are more straightforward
and familiar to users of those functions.
...
of the arguments in a function. There are a number of reasons for this be
the most obvious it that R will often produce errors when give an
...
Quosures give you a way of preventing R from doing a standard evaluation and
gives you access to the text of the arguments passed into a function, like this:
...
R do a standard evaluation of `expr`.

And you can get the literal text of the expression. In theory that is all you 
need to interpret the expression like `rlang::filter does with its arguments. In
practice though the raw text is to hard work with most of the time. And there
are better way access the information in the expression, which we will see as
this tutorial progresses.

`rlang::enquo` is one of many functions that produce a quosure. The quosure
gives you access to a number of things. One is the literal text of an 
...
simple terms for R an object model is just an elaborate `list` that breaks out
