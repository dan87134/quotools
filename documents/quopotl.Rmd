...
barithmetic <- function(expr) {
	...
	model_table <- qtls_make_ast_table(q)
	# we only want to use car model objects
	cars <- dplyr::filter(model_table, expr_property == "car")
...
q <- rlang::quo(a * b + 2 * d )
ast_tbl <- qtls_make_ast_table(q)
g <- qtls_plot_ast(ast_tbl)
DiagrammeR::render_graph(g, layout = "tree")
```
What you see here is the abstract syntax table, or AST, for the `a * b + c * d` expression. We'll cover more about AST's shortly.
