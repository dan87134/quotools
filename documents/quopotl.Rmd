...
R doesn't actually know what to do with `1 + 2 + 3` as is. It has translate it
to something is does know about, the nested `+` functions in the first example.
...
But the R syntax for chosing rows has more of what is sometimes called *programming ceremony*
...
tree table. That's something we'll cover in detail later.
...
of a quosure. In practice, though, you will be using what is called a path to
...
to the right of the colon in the nodes along the way to node 4.
...
```{r node_access}
# note that the argument to [[ is an numeric vector
node4 <- q[[path_to_4]]
node4
```
...
```
Here you can see that you can use an outline tree to get specific and detailed
information about the objects in the object model of a quosure.
...
##Trees and Paths and Immutable Objects

We've seen in the preceding examples that a quosure is a tree of objects that
represents an expression. We've also seen that the tree itself is lists of lists
that we can explore by using subsetting, for example we can access one of the
`+` operators in `a + b + c` using subsetting like this:
...
But we also talked about how using a path is a convienient way to access
objects in the object model like:
...
Both of these ways of accessing a `+` operator in `a + b + c` seem to be equivalent
but they are not. Using a path is necessary if you are going to modify a quosure
as we did when we implemented Bizzarro World arithmetic. We can see that here...
...
... that sequentually getting object after object using subsetting
did not change q.
But...
...
This isn't due to some quirk in the way quosures work. It's due to a fundamental
property of R, objects are immutable. That means that once you set the
value of an object you can't change it.

You might question this property though because the following seems to work just 
fine.
...
It sure seems like we *changed* the value of `a`, but in fact we
didn't. We'll see that shortly.

Often the `<-` operator is sometimes called the assignment operator
and the expression `a <- 9` is sometimes spoken as "9 is assigned
to a".

A better way to say what `a <- 9` does is "create a new variable named
'a' and initialize its value to 9."

By looking at some of the innards of R we can see that `<-` creates a 
new object:
...
`pryr::address` returns the memory location of an object. Look closely and you
will see that the address of `a` changed when `10` was *assigned* to it. The
address changed because `<-` created a new object for `a`.

This immutable property is true even when subsetting is involved.

```{r immut4}
# make a numeric vector with 10 elements in it
nm <- 1:10
# get it address
pryr::address(nm)
# now change a location in the vector
nm[2] <- -2
## and get it's address again
pryr::address(nm)
# and nm has moved
```

So with our new description of what the `<-` operator does lets look at the
example we started with and the addresses involved.

```{r path9}
# make a new object named q and initialize it with a quosure
q <- rlang::quo(a + b + c)
pryr::address(q)
# make a new object named obj1 and initialize it with the content of q[[2]]
obj1 <- q[[2]]
# make a new object named obj2 and initialize it with the content of obj1[[1]]
obj2 <- obj1[[1]]
# make a new object named obj2 and initialize it with a symbol
obj2 <- rlang::sym("-")
# So we didn't  change q, we just created a new objects.
# So the fact that q didn't change should be expected.
pryr::address(q)
q
```

A good clue that we didn't change `q` is that it's address after 
`obj2 <- rlang::sym("-")` is the same as it was at the beginning
of the script. If `q` had been changed it would have had a 
different address.

Now lets do the same thing with the example that uses a path.

```{r path7}
# make a new object named q and initialize it with a quosure
q <- rlang::quo(a + b + c)
pryr::address(q)
# make a new object named `q` that is the same as the current object
# named `q` but with a different conent
q[[c(2,1)]] <- rlang::sym("-")
pryr::address(q)
q
```

In this example the address of `q` did change. So we ended up with a brand
new quosure but its object model is slightly different than the one we
started with, that's why it contains the `-` we wanted it to.

Just using a path to access the object we want to change isn't the only
requirement to modify a quosure. The subsetting has to be done on the left
hand side of the `<-` operator. Here is what happens if you try to do things
...
q <- rlang::quo(a + b + c)
pryr::address(q)
# make a new object named obj and initialize it with q[[c(2,1)]]
obj <- q[[c(2,1)]]
# make a new object named obj and initialize it as a symbol
obj <- rlang::sym("-")
pryr::address(q)
q
```

Here we can see that changing `obj` did not change the address of `q` nor did
it change the content of `q`. That is because using a path on the right
hand side of `<-` just created a new object that was not part of `q'.

So in order to change a part of the object model of a quosure with subsetting
you have to use a selector with a path in it on the right hand side of
a `<-` operator.

## Unwrappping Quosures
Once you start working with quosures you will run into problems where you
have a quosure but you need an expression. Here is an example:

```{r unwrap1}
# function that returns the arithmetic ops of an expression
...
You can see that our `get_arith_ops` worked with a literal expression passed
into it.

But what if instead of a actual expression what you have is quosure? 
...
As you can see you can't just pass the quosure into `get_arith_ops`. What you
need is the expression from the quosure q.

This is the problem that *unwrapping* solves. A special operator `!!` unwraps
an expression. This operator is sometimes called the double bang operator.
...
get_arith_ops(!! q)
```
And this works because `!! q` passed in the expression from `q` not `q` itself.

But we already know how to get an expression from a quosure, that's what the 
rlang::get_expr does for us. But that won't work

```{r unwrap4}
# function that returns the arithmetic ops of an expression
q <- rlang::quo(a + b * c)
#try it out here
get_arith_ops(rlang::get_expr(q))
```

That didn't work because the first thing that `get_arith_ops` does is 
`rlang::enquo` what was passed into it, so what it did, in effect, was
`rlang::enquo(rlang::get_expr(q))', that is it quoted the expression
that was passed into it. That's why `!!` is special, it doesn't get
quoted, the expression inside `q` is quoted.

`!!` is also special because it only works on arguments of functions that 
get passed into a function that creates a quosure. So R won't be able to 
...
There is another kind of unwarping problem you can run into.

`dplyr::select` is an NSE function. You pass into it
columns you want to select and possibly rename. Here is an example:
...
dplyr::select(tbl, ex = x, y)
```

But it's quite possible that you may, as part of your applications, create a
list of names of columns, for example you might get them from user input. For
example your application might create a list like this:

```{r wrap11}
columns <- list(ex = "x", "z")
```

You can't pass this list into `dplyr::select` ...

```{r wrap12, error=TRUE}
dplyr::select(tbl, columns)
```

The is another quoting operator that can, in effect, turn a list like this into
a literal, comma separated list if values. The is will work only for an NSE
function that has `...` as one its arguments. A quick check of `dplyr::select`
...
That operator is the `!!!` operator, sometimes called the tripple bang operator.
It's used like this:
...
And we see that we got the results we wanted, including the renaming of
the column.

This works because somewhere inside the `dplyr::select` function it uses
`rlang::quos(...)` to make a quosure out of the `...` argument. The `!!!`
operator makes `rlang::quos` turn the list in to, in effect, a literal,
comma separated list of arguments and then makes a quosure out of that.


# Quosure Tools

The code for the qstl_ functions used in this tutorial is available at 
https://github.com/dan87134/exquotools.git. 
