...
This is using the add function builtin to R. This function takes only two
arguments. You have to nest the functions in order to add more than two
arguments.
...
This familiar, arithmetic syntax works because under the covers R converts it 
to nested `+` functions so that it can evaluate it. 

Now R doesn't use NSE to do this because the implementors of R we able to
include arithmetic as a native syntax to R. But the important take away from
this example is that including this arithmetic syntax in R didn't extend R's computational capabilities, it just made it easier to use it in the problem domain
of doing arithmetic calculations.

Using quosures to *Program on the Language* lets you do this too...
but without
changing the implementation of R, you just write ordinary functions that
use quosues. This way you can make R easier to use in the problem domains you
deal with.
...
that this list of lists of lists... is a tree structure.
...
by accessing the list of lists of lists... in the quosure.

We've glossed over the details of how or why `f4` works but we'll get to them as we go 
...
You can work with quosures without using a path but it is tedious and error
prone to do so.
...
just replace it in most cases. You have to, in effect, just add to it. In
most cases the environment used by a quosure ins't private to it, it is
