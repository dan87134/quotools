...
all the aritmetic operators in it, that is +, -, *, and /, and then flips each one to
...
bizzarro_op <- tibble::tribble(~ sym, ~ bizzarro_sym,
	...
	"*" , "/",
	"/", "*")
# uses bizzarro_table to flip arithmetic operators
...
in the expression and then let R do the evaluation of the quosure instead
of overrding the arithmetic operators. rlang::eval_tidy is doing the
evaluation of the quosure.

The function `qtls_make_rlang_table` converts the model of an R expession that 
the `rlang` packages uses into a table. And the only rows we care about
are ones where the expression is a symbol... bizzarro flip checks for that
by using rlang::is_symbol.

Here is what that table looks like for the 
second bizzarro calculation we did.


We'll be covering more details of what is in this table later but here are
...
The only column we care about for implementing bizzarro arithmetic is the
expression. 
...
You can see that it is a tree structure, the id and parent columns define the
tree. Here is a visualization of the basic tree structure
...
the model `rlang` uses to represent a quosure is a three of expressions.

A the root of the tree you can see the expression we started with
`a * b + c * d`. As you move down the tree you can see that it breaks
down the initial expression into smaller an smaller pieces.

There is a special kind of tree that R uses it do the actual evaluation
of a quosure. It is called a abstract syntax tree or AST. Even though
is is tree shaped an AST is, in effect, a detailed list of instruction
that R uses to do the actual evaluation.
