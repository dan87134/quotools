...
```{r dsl1}
#
	my_negate <- function(expr) {
		-expr
	}

# use negate to negate the sum of 1, 2 and 3
my_negate(`+`(`+`(1, 2), 3))
```
...
that is the standard syntax for arithmetic.
...
to break an expression into an object module. In programming this is called
...
what we need to do is convert an quosure to a table. That's what the
...
It does not depend on primative functions that traverse linked lists, are
tedious to use at best, and produce code that is hard to reason about.


## Tree Tables and Paths

It's easy to use a tree table for many things.
For example the `qtls_plot_model` function to make a diagram of a quosure
that can be rendered by `DiagrammeR`. Here is an example:
...
use that path to *lookup* the that node for the row the path is in. 
