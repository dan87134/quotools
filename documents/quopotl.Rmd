...
expression in function argument so that you can evaluate it any way you
want. This called non-standard evaluation, or NSE, of an argument because
...
is that adding arithmetic syntax to R didn't add anything to the computational
capabilites of R, it just made it easier to use in the problem domain of
doing arithmetic calculations.

Using quosures to *Programming on the Language* lets you add
...
to type into an expression that has little to do with telling R what it is
...
The error occurs because `'as.character` function use R do a standard
evaluation of `expr` before it tried to process it. When R did that
...
to interpret the expression like `rlang::filter does with its arguments and
...
simple terms a quosure acts mimics a `list` that breaks out
...
expressions. And the really important thing here is that **you** don't write the
code that does the parsing... in fact that would be very
...
we use the `rlang::get_expr` function we used two examples ago to she what
...
This lets you embed your own custom syntax into the arguments of function. An
...
functions to be process. The is a computer science term for this *object model*.
...
In this example we will make use of the fact that a quosure includes a
context that is used when the quosure is evaluated. 
...
just replace it. You have to, in effect, just add to it. 
...
barithmetic <- function(expr) {
	...
	# so this doesn't change these definitions anywhere else.
