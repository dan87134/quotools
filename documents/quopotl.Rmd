...
barithmetic <- function(expr) {
	...
	# we only want to use car model objects
	cars <- dplyr::filter(model_table, type == "car")
	# if the car is an arithmetic operation 
	# we want to flip it to the bizzarro  world
	purrr::walk(cars$expression, function(expr) {
...
the `rlang` packages uses into a tabular form that is easier to use when you are
"programming on the language". It lets you apply a lot of your R table processing
