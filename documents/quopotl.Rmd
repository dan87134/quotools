...
that in memory repesentation of the module. In fact the `rlang` is a wrapper around
the `c` language memory model that R uses to evaluate expressions.

So a quosue gives you access to all three of these language operational
components.
...
not tags, the are markers that represent objects used in the in an in memory model
of html.
...
was to request the web site that
...
as using javascript with html to make dynamic web pages.
...
action. These examples are only meant to show what can be accompished with quosures, the real programming details of how this is done is covered later in the tutorial.
...
in order to implement this measure and it uses a quosure to do this.
...
from the files that can be downloaded for this project. The details of how the
functions in this code works won't be explained until later in this tutorial.
...
barithmetic <- function(expr) {
	q <- rlang::enquo(expr)
	model_table <- qtls_walk_carcdr(q)
	cars <- dplyr::filter(model_table, type == "car")
	x <- purrr::walk(cars$expression, function(expr) {
...
The function `qtls_walk_carcdr` converts the model of an R expession that 
the `rlang` uses into a tabular form that is easier to use when you are
"programming on the language". Here is what that table looks like for the 
